...
| The clip advances on the PIE component: what was ruled out | investigated | Five candidates eliminated with evidence, none of them the cause. (1) Pose ticking: the default is `ONLY_TICK_POSE_WHEN_RENDERED` and nothing is rendered under `-nullrhi`, so `visibility_based_anim_tick_option` was set to `ALWAYS_TICK_POSE_AND_REFRESH_BONES` (that is the name on this series, not `..._BONE_TRANSFORMS`) along with `no_skeleton_update`, `enable_update_rate_optimizations` and `set_component_tick_enabled`: all five applied, the reading stayed 0.0. (2) Wrong mode: the PIE component reports `ANIMATION_SINGLE_NODE`. (3) No anim instance: `get_anim_instance()` returns one. (4) The world not ticking: the character walks 326 cm under `add_movement_input`, so it ticks. (5) The reading API: switched from `get_socket_location`, which also moves when the actor walks, to `get_delta_transform_from_ref_pose`, which cannot; same 0.0. **Next to try**: a plain `SkeletalMeshActor` instead of a `Character`, whose mesh component may have its pose driven elsewhere |
